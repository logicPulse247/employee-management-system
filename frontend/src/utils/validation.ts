...
export const employeeSchema = z.object({
  ...
  subjects: z
    ...
    .refine(arr => arr.some(s => s && s.trim().length > 0), 'At least one subject is required'),
  attendance: z
    .number({
...
export const employeeUpdateSchema = z.object({
  ...
  class: z
    .string({ required_error: 'Class is required', invalid_type_error: 'Class must be a string' })
